...
class SessionDataLoader:
    ...
    def load_freemocap_unix_timestamps(self):
        ...
        camera_names = [
            column_name
            for column_name in list(freemocap_unix_timestamps_dataframe.columns)
            if "Cam" in column_name
        ]

        for this_camera_name in camera_names:
            each_cam_timestamps_list.append(
                np.array(freemocap_unix_timestamps_dataframe[this_camera_name])
            )

        cameras_timestamp_array = np.stack(each_cam_timestamps_list, axis=1)
        cameras_timestamp_array[
            cameras_timestamp_array == -1
        ] = np.nan  # replace -1 with nan
    ...
    def load_mediapipe_data(
        ...
        if self._session_path is None:
            raise ValueError(
                "Session path is not specified - Can't load mediapipe data without a session path"
            )

        mediapipe_data_path = (
            self.session_path / "DataArrays" / "mediaPipeSkel_3d_smoothed.npy"
        )
        ...
        if move_to_origin:
            mean_position_xyz = np.nanmedian(
                np.nanmedian(mediapipe_fr_mar_xyz, axis=0), axis=0
            )
...
if __name__ == "__main__":
    ...
    print(
        "pupil data loaded, but I don't have an easy way to print out the shape of things yet, lol"
    )
