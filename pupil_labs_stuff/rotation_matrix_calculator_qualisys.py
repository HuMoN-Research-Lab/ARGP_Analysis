...
class RotationMatrixCalculator:
    ...
    def __init__(self, skelly_dict: dict):
        self._skelly_dict = skelly_dict
    ...
    def calculate_head_rotation_matricies(
        ...
        head_rotation_data = self.define_basis_vectors(
            center_point_fr_xyz=self._skelly_dict['head_center_xyz'],
            x_direction_fr_xyz=self._skelly_dict['head_front_xyz'],
            y_direction_fr_xyz=self._skelly_dict['head_left_xyz'],
    ...
    def show_head_rotation_debug_plot(self, rotation_data: RotationDataClass):
        ...
        for key, value in self._skelly_dict.items():
            z_skelly_dict[key] = pin_point_new_origin(value, local_origin_fr_xyz)
        ...
        for frame_number in range(trail_length, x_hat_xyz.shape[0], 10):
            ax.clear()

            print(f'debugging frame {frame_number}')

            # plot head orthonormal basis vectors
            ax.plot(
                x_hat_xyz[frame_number - trail_length: frame_number, 0],
                x_hat_xyz[frame_number - trail_length: frame_number, 1],
                x_hat_xyz[frame_number - trail_length: frame_number, 2],
                "r-",
            )

            ax.plot(
                y_hat_xyz[frame_number - trail_length: frame_number, 0],
                y_hat_xyz[frame_number - trail_length: frame_number, 1],
                y_hat_xyz[frame_number - trail_length: frame_number, 2],
                "g-",
            )

            ax.plot(
                z_hat_xyz[frame_number - trail_length: frame_number, 0],
                z_hat_xyz[frame_number - trail_length: frame_number, 1],
                z_hat_xyz[frame_number - trail_length: frame_number, 2],
                "b-",
            )
            ...
            ax.plot(
                test_point_xyz_rot[frame_number - trail_length: frame_number, 0],
                test_point_xyz_rot[frame_number - trail_length: frame_number, 1],
                test_point_xyz_rot[frame_number - trail_length: frame_number, 2],
                "k-",
            )
            ...
            for key, value in z_skelly_dict.items():
                ax.scatter(
